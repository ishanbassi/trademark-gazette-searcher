...
export async  function fullTmSearch(tmArray:TmInterface[]) {
    const keywordPhonetic  = tmArray.map(tm => {
        return DoubleMetaphone.process(tm.trademark).join(" ")
    })

    const tms = await db('tm_detail')
    .whereRaw('trademark = ? ', tmArray.map(tm => tm.trademark))
...
fullTmSearch([{trademark:'JAI MAA', tmClass:"45"}]).then(res => console.log(res))
