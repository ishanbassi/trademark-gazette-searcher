...
async function fillCaptcha(page:Page , captchaText:Promise<string>) {
    ...
    let el1 = await page.evaluate((el:HTMLElement) => el.previousElementSibling , td)
    let el2 = await page.evaluate((el:HTMLElement) => el.parentElement , td)
    let el3 = await page.evaluate((el:HTMLElement) => el.nextElementSibling , td)
    let el4 = await page.evaluate((el:HTMLElement) => el.nextSibling , td)
    let el5 = await page.evaluate((el:HTMLElement) => el.nextElementSibling , td)
