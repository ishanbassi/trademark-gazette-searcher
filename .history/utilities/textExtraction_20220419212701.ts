...
export async function extractPdfText(filePath:string, options?:PDFExtractOptions) {
    const content = []
    let tms = []
    ...
    await promises.writeFile('pdf2.json', JSON.stringify(tms))
}
...
extractPdfText('./pdfs/journal 2044/trademark_gazette.pdf').then(res => console.log(res)).catch(err => console.log(err))