        ...
        pdfExtract.extract(filePath,options,  (err, data) => {
            ...
            data.pages.forEach( (page, i) =>{
                ...
                page.content.forEach( (data,i) => {
                    ...
                    if (data.height >= 23 && data.height <= 24 && data.x >= 58 && data.x <= 59 && data.y >= 128 && data.y <= 138){
...
extractPdfText('./pdfs/journal 2044/trademark_gazette.pdf').then(async res => await promises.writeFile('pdf1.json', JSON.stringify(res)) ).catch(err => console.log(err))