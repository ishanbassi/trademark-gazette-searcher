        ...
        pdfExtract.extract(filePath,options,  (err, data) => {
            ...
            data.pages.forEach((page, i) =>{
                ...
                if (trademark) {
                    ...
                    const[ ,journal,tmClass] = journalAndClass.match(/Trade Marks Journal No:\s+(\d\d\d\d).+Class\s+(\d)/)
                    
                    content.push({
                        "page_no":`${page.pageInfo.num},journal no. ${journal}`,
                        trademark,
...
extractPdfText('./pdfs/journal 2045/document5.pdf').then(async res => await promises.writeFile('pdf1.json', JSON.stringify(res), {encoding:'utf-8'}))