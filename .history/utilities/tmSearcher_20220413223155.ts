...
export async  function fullTmSearch(tmArray:TmInterface[]) {
    ...
    const result = await Promise.all(tmArray.map(tm => {
        const tmPhonetics = Metaphone.process(tm.trademark)
        const wordsList = tm.trademark.split(' ')
        const queryResult = db('tm_detail').select(['page_no', 'details', 'tm_class', 'trademark', db.raw(`? as regTm`, tm.trademark)])
    ...
    await closeConnection()
    const a = result.reduce((prevArr, currArr) => prevArr.concat(currArr))
    return a
