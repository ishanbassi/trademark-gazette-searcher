...
export async  function fullTmSearch(tmArray:TmInterface[]) {
    ...
    const result = await Promise.all(tmArray.map(async tm => {
        const tmPhonetics = Metaphone.process(tm.trademark)
        const wordsList = tm.trademark.split(' ')
        const queryResult = await db('tm_detail').select(['page_no', 'details', 'tm_class', 'trademark', db.raw(`? as regTm`, tm.trademark)])
                .where('trademark', tm.trademark)
                .orWhereILike('trademark', `%${tm}%`)
                .orWhereIn('trademark' , wordsList)
                .orWhere('tm_phonetics', tmPhonetics)
                .orWhereLike('tm_phonetics', `_${tmPhonetics}_`)
        return queryResult
    }))
    await closeConnection()
    
    return result.reduce((prevArr, currArr) => prevArr.concat(currArr))
