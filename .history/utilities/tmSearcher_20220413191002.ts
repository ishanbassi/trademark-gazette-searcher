...
export async  function fullTmSearch(tmArray:TmInterface[]) {
    ...
    const result = await Promise.all(tmArray.filter( async tm => {
        
        const tmPhonetics = Metaphone.process(tm.trademark)
        const wordsList = tm.trademark.split(' ')
        const  currentTmSearch = await db('tm_detail').select(['page_no', 'details', 'tm_class', 'trademark', db.raw(`${tm.trademark} as regTm`)])
            ...
            if( currentTmSearch.length !== 0) return Promise.resolve(currentTmSearch)
    }))
    await closeConnection()
    return result
