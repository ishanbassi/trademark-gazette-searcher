        ...
        pdfExtract.extract(filePath,options, async  (err, data) => {
            ...
            data.pages.forEach( (page, i) =>{
                ...
                page.content.forEach( (data,i) => {
                    ...
                    if (data.height >= 23.94 && data.height ){
                        tms.push(data)
            ...
            await promises.writeFile('pdf3.json', JSON.stringify(tms))
        })
...
extractPdfText('./pdfs/journal 2045/document6.pdf').then(res => console.log(res)).catch(err => console.log(err))