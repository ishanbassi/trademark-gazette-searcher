...
const  App:FunctionComponent  = (props) =>  {
    ...
    const fileUpload =   async (xlsFile:File) => {
        ...
        const tmCell = {}
        loop1:
        for (let i=0; i< Math.ceil(Object.keys(sheetData).length/4) ; i++){
            loop2:
            for(let alphabet of alphabets.split('')) {
                const cellValue:string =  sheetData[`${alphabet}${i+1}`].w.toLowerCase()
                if(tmCellRegExp.test(cellValue)) tmCell['tm'] = alphabet
                else if(tmClassCellRegExp.test(cellValue)) tmCell['class'] = alphabet

                if(tmCell['tm'] && tmCell['class']) break loop1;
            }
