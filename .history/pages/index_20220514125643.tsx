...
const  App:FunctionComponent  = (props) =>  {
    ...
    useEffect( () => {
        ...
        if(loading) {
            let currentJournal = journals.current
            const urlPath = `/api/fileReader?journal=${currentJournal}`
    ...
    return(
        <>
            ...
            <Container className="mt-5" fluid>
                ...
                <Form.Select id="journals">
                    {journals.current.map((journal, i) => {
                        let journal_no = journal.journal_no
                        return(
                            <option value={journal_no} key={`${journal_no}_${i}`} onChange={() => setLoading(true)}>{journal_no}</option>
                        )
                    })}
                </Form.Select>
