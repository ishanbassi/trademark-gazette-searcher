...
const  App:FunctionComponent  = (props) =>  {
    ...
    const journals = useRef({journal_no:null, first_journal:null})
    const tmClassArr = useRef([])
    
    useEffect(() => {
        fetch('/api/fileReader', {method:'GET'})
        .then(res => res.json())
        .then(data => journals.current = data[0])
    }, [])
    useEffect( () => {
        
        if(loading) {
            console.log(journals.current)
            const urlPath = `/api/fileReader?journal=${journals.current.journal_no}`
            ...
            journals.current = {
                journal_no: journals.current.journal_no - 1,
                first_journal: journals.current.first_journal
            }
        }
    },[loading]
    ...
    return(
        <>
            ...
            {journals.current.journal_no  >= journals.current.first_journal ? 
             <Container  fluid="md" className="text-center mt-2">
                <Button
