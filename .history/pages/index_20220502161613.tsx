...
const  App:FunctionComponent  = (props) =>  {
    ...
    const fileUpload =   async (xlsFile:File) => {
        ...
        const tmClassArr = []
        const cells = sheetData["!ref"] ? parseInt(sheetData["!ref"].match(/\d+/)[0]) : null
        console.log(sheetData["!ref"].match(/\d+/)[0])
        ...
        if(tmAndClassCellAlphabet['tm'] && tmAndClassCellAlphabet['class']) {
            ...
            for(let i=0;i< cells ; i++) {
                ...
                tmClassArr.push(
                    {
                        'trademark':sheetData[`${tmAndClassCellAlphabet['tm']}${cells+1}`].w,
                        'tmClass':sheetData[`${tmAndClassCellAlphabet['class']}${cells+1}`].w
                    }
                )
            }
        }
