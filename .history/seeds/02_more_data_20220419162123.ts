...
export async function seed(knex: Knex): Promise<void> {
    ...
    // Inserts seed entries
        ...
        const pdfDirIterable:Generator = generator('./pdfs')
        while((await pdfDirIterable.next()).done === false){
            
            const trademarks = await extractPdfText((await pdfDirIterable.next()).value)
